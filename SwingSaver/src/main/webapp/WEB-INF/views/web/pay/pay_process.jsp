...
<%
  request.setCharacterEncoding( "utf-8" );
    ...
    String req_tx          = f_get_parm( (String)request.getAttribute( "req_tx"         ) ); // 요청 종류         
    String res_cd          = f_get_parm( (String)request.getAttribute( "res_cd"         ) ); // 응답 코드         
    String tran_cd         = f_get_parm( (String)request.getAttribute( "tran_cd"        ) ); // 트랜잭션 코드     
    String ordr_idxx       = f_get_parm( (String)request.getAttribute( "ordr_idxx"      ) ); // 쇼핑몰 주문번호   
    String good_name       = f_get_parm( (String)request.getAttribute( "good_name"      ) ); // 상품명            
    String good_mny        = f_get_parm( (String)request.getAttribute( "good_mny"       ) ); // 결제 총금액       
    String buyr_name       = f_get_parm( (String)request.getAttribute( "buyr_name"      ) ); // 주문자명          
    String buyr_tel1       = f_get_parm( (String)request.getAttribute( "buyr_tel1"      ) ); // 주문자 전화번호   
    String buyr_tel2       = f_get_parm( (String)request.getAttribute( "buyr_tel2"      ) ); // 주문자 핸드폰 번호
    String buyr_mail       = f_get_parm( (String)request.getAttribute( "buyr_mail"      ) ); // 주문자 E-mail 주소
    String use_pay_method  = f_get_parm( (String)request.getAttribute( "use_pay_method" ) ); // 결제 방법          
	String enc_info        = f_get_parm( (String)request.getAttribute( "enc_info"       ) ); // 암호화 정보       
    String enc_data        = f_get_parm( (String)request.getAttribute( "enc_data"       ) ); // 암호화 데이터     
    String cash_yn         = f_get_parm( (String)request.getAttribute( "cash_yn"        ) );
    String cash_tr_code    = f_get_parm( (String)request.getAttribute( "cash_tr_code"   ) );
    /* 기타 파라메터 추가 부분 - Start - */
    String param_opt_1    = f_get_parm( (String)request.getAttribute( "param_opt_1"     ) ); // 기타 파라메터 추가 부분
    String param_opt_2    = f_get_parm( (String)request.getAttribute( "param_opt_2"     ) ); // 기타 파라메터 추가 부분
    String param_opt_3    = f_get_parm( (String)request.getAttribute( "param_opt_3"     ) ); // 기타 파라메터 추가 부분
    /* 기타 파라메터 추가 부분 - End -   */

  String tablet_size     = "1.0"; // 화면 사이즈 고정
  String url             = g_url;  // request.getRequestURL().toString();
...
<script type="text/javascript">
  ...
  function init_orderid()
  {
    ...
    var order_idxx = "SWING" + year + "" + month + "" + date + "" + time;
...
<form name="order_info" method="post">
  ...
  <div class="sample">
    ...
    <table class="tbl" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <th>전화번호</th>
        <td><input type="text" name="buyr_tel1" class="w100" value="${payInfo.buyr_tel1}"></td>
      </tr>
      <tr>
        <th>휴대폰번호</th>
        <td><input type="text" name="buyr_tel2" class="w100" value="${payInfo.buyr_tel2}"></td>
      </tr>
    </table>
  ...
  <input type="hidden" name="Ret_URL"         value="<%=url%>/pay/process/${payInfo.artype}/${payInfo.arid}">
  <!-- 화면 크기조정 -->
  <input type="hidden" name="tablet_size"     value="<%=tablet_size%>">

  <!-- 추가 파라미터 ( 가맹점에서 별도의 값전달시 param_opt 를 사용하여 값 전달 ) -->
  <input type="hidden" name="param_opt_1"     value="${payInfo.order_idx}">
  <input type="hidden" name="param_opt_2"     value="${payInfo.artype}">
  <input type="hidden" name="param_opt_3"     value="${payInfo.arid}">
...
<form name="pay_form" method="post" action="/pay/process/payclihub"> <!-- pp_cli_hub.jsp"  -->
