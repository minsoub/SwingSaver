...
<%
    ...
    String tran_cd        = f_get_parm( (String)request.getAttribute( "tran_cd"        ) ); // 처리 종류
    /* = -------------------------------------------------------------------------- = */
    String cust_ip        = f_get_parm( (String)request.getRemoteAddr()                  ); // 요청 IP
    ...
    String good_mny      = f_get_parm((String) request.getAttribute( "good_mny"      ) ); // 상품금액
    
    /* = -------------------------------------------------------------------------- = */
    String res_cd         = "";                                                     // 응답코드
    String res_msg        = "";                                                     // 응답 메세지
    String tno            = f_get_parm( (String)request.getAttribute( "tno"            ) ); // KCP 거래 고유 번호
    /* = -------------------------------------------------------------------------- = */
    ...
    /* = -------------------------------------------------------------------------- = */
    String cash_yn        = f_get_parm( (String)request.getAttribute( "cash_yn"        ) ); // 현금 영수증 등록 여부
    String cash_authno    = "";                                                     // 현금 영수증 승인 번호
    String cash_tr_code   = f_get_parm( (String)request.getAttribute( "cash_tr_code"   ) ); // 현금 영수증 발행 구분
    String cash_id_info   = f_get_parm( (String)request.getAttribute( "cash_id_info"   ) ); // 현금 영수증 등록 번호
    String cash_no        = "";                                                     // 현금 영수증 거래 번호      

    String param_opt_1    = f_get_parm( (String)request.getAttribute( "param_opt_1" ) );
    String param_opt_2    = f_get_parm( (String)request.getAttribute( "param_opt_2" ) );
    String param_opt_3    = f_get_parm( (String)request.getAttribute( "param_opt_3" ) );
...
<form name="pay_info" method="post" action="/pay/process/result">
